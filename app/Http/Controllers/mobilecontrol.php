...
class mobilecontrol extends Controller
{
	...
	function registrasi(Request $req){
		...
		if($user->save()){
			...
			echo json_encode(["type"=>"error","msg"=>"Data Success Disimpan ! "]);
		}
	}
	...
	function login(Request $req){
		...
		if($ceklogin){
					if(Hash::check($login["password"],$ceklogin->password)){
								...
								echo json_encode(["type"=>"error", "msg"=>"Password Tidak Valid !!"]);
					}
		}else{
					echo json_encode(["type"=>"error","msg"=>"Email Tidak Valid !!"]);
		}
